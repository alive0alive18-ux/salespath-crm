  ...
  return(
    <div style={{display:'flex',minHeight:'100vh',background:CREAM,fontFamily:"'DM Sans','Apple SD Gothic Neo',system-ui,sans-serif",fontSize:14}}>
      ...
      <aside style={{width:210,background:NAVY,display:'flex',flexDirection:'column',flexShrink:0,height:'100vh',position:'sticky' as const,top:0}}>
        ...
        <div style={{paddingTop:8,flex:1,overflowY:'auto' as const}}>
          {nav.map(n=>(
...
function ClientMap({clients}:any){
  ...
  useEffect(()=>{
    ...
    geocoded.forEach((c,i)=>{
      ...
      m.addListener('click',()=>{
        infoWin.current.setContent(`<div style="padding:10px;min-width:180px;font-family:sans-serif"><div style="font-size:16px;font-weight:800;color:#000;margin-bottom:6px">${c.name}</div><div style="font-size:13px;color:#222;font-weight:700;color:#111;margin-bottom:4px">${stg.label} · ${c.interest_model||c.car_model||'차종 미정'}</div><div style="font-size:12px;color:#444">${c.address}</div>${c.phone?`<a href="tel:${c.phone.replace(/-/g,'')}" style="display:inline-block;margin-top:8px;padding:4px 12px;background:#1B2A4A;color:#fff;border-radius:4px;font-size:12px;text-decoration:none">📞 전화</a>`:''}</div>`)
    ...
    <div>
      ...
      {selected&&(
        <div style={{...card,marginTop:16}}>
          <div style={cardH}>
            <span>{selected.name} 고객</span>
            <div style={{display:'flex',gap:8}}>
              {selected.phone&&<a href={`tel:${selected.phone.replace(/-/g,'')}`} style={{...btn('navy'),textDecoration:'none',display:'flex',alignItems:'center',gap:6}}>📞 전화</a>}
...
function MobileApp({page,setPage,user,salesperson,setSalesperson,clients,setClients,schedules,weekSchedules,templates,setTemplates,partners,setPartners,selectedClient,setSelectedClient,signOut}:any){
  ...
  const startDuty=async()=>{
    const{data:{user:u}}=await supabaseMobile.auth.getUser()
    if(!u) return
    await supabaseMobile.from('salespersons').update({is_on_duty:true,duty_start:new Date().toISOString()}).eq('id',u.id)
    setIsOnDuty(true)
  }
  ...
  const endDuty=async()=>{
    ...
    if(!u) return
    const today=new Date().toISOString().split('T')[0]
    const{data}=await supabaseMobile.from('clients').select('*').eq('duty_date',today).eq('salesperson_id',u.id)
    setDutyClients(data||[])
    await supabaseMobile.from('salespersons').update({is_on_duty:false,duty_start:null}).eq('id',u.id)
    setIsOnDuty(false)
